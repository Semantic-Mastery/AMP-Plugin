...
require( dirname( __FILE__ ) . '/class-ampscreator-content.php' );

class AMPsCreator_Post {
	private $ID;
	private $post;
	private $author;
	private $content;
	...
	function __construct( $post_id ) {
		...
		$this->content = apply_filters( 'amps_post_content', $this->build_content(), $this->post );
		$this->metadata = apply_filters( 'amps_post_metadata', $this->build_metadata(), $this->post );
	}
	...
	function get_author_avatar( $size = 24 ) {
		...
		$converter = new AMPsCreator_Img_Converter( $avatar_html );
	...
	private function build_content() {
		$amps = new AMPsCreator_Content( $this->post->post_content );
		$content = $amps->transform();
		$this->scripts = $amps->get_scripts();
		return $content;
	}
