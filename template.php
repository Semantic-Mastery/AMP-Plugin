...
<html amps>
<head>
	<title><?php echo esc_html( $amps_post->get_title() ); ?> | <?php echo esc_html( get_bloginfo( 'name' ) ); ?></title>
	<meta charset="utf-8">
	<link rel="canonical" href="<?php echo esc_url( $amps_post->get_canonical_url() ); ?>" />
	...
	<?php foreach ( $amps_post->get_scripts() as $element => $script ) : ?>
		<script custom-element="<?php echo esc_attr( $element ); ?>" src="<?php echo esc_url( $script ); ?>" async></script>
	<?php endforeach; ?>
	<script src="https://cdn.ampsproject.org/v0.js" async <?php echo defined( 'AMPsCreator_DEV_MODE' ) && AMPsCreator_DEV_MODE ? 'development' : ''; ?>></script>
	<script type="application/ld+json"><?php echo json_encode( $amps_post->get_metadata() ); ?></script>
	<?php do_action( 'amps_head', $amps_post ); ?>
	<style>body {opacity: 0}</style><noscript><style>body {opacity: 1}</style></noscript>
	<style amps-custom>
	/* Generic WP styling */
	amps-img.alignright { float: right; margin: 0 0 1em 1em; }
	amps-img.alignleft { float: left; margin: 0 1em 1em 0; }
	amps-img.aligncenter { display: block; margin-left: auto; margin-right: auto; }
...
<div class="content">
	<h1 class="title"><?php echo esc_html( $amps_post->get_title() ); ?></h1>
	<ul class="meta">
		<li class="byline">
			<?php echo $amps_post->get_author_avatar(); ?>
			<span class="author"><?php echo $amps_post->get_author_name(); ?></span>
			<span>&nbsp;&nbsp;&bull;</span>
		</li>
		<li><time datetime="<?php echo esc_attr( $amps_post->get_machine_date() ); ?>"><?php echo esc_html( $amps_post->get_human_date() ); ?></time></li>
	</ul>
	<?php echo $amps_post->get_content(); ?>
</div>
